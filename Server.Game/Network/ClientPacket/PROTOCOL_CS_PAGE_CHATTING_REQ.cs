    ...
    public class PROTOCOL_CS_PAGE_CHATTING_REQ : GameClientPacket
    {
        ...
        public override void Run()
        {
            try
            {
                ...
                if (player == null || this.Field0 != ChattingType.ClanMemberPage || ComDiv.GetDuration(player.LastChatting) < 1.0)
                    return;
                ...
                if (player.IsMuted())
                {
                    Client.SendPacket(new PROTOCOL_SERVER_MESSAGE_ANNOUNCE_ACK("You are muted and cannot use clan page chat."));
                    return;
                }
                using (PROTOCOL_CS_PAGE_CHATTING_ACK Packet = new PROTOCOL_CS_PAGE_CHATTING_ACK(player, this.Field1))
                    ClanManager.SendPacket(Packet, player.ClanId, -1L, true, true);
