...
namespace Server.Game.Network.ClientPacket
{
    ...
    {
        private List<MatchModel> Field0 = new List<MatchModel>();
        private int Field1;
                        ...
                        lock (channel.Matches)
                        {
                            foreach (MatchModel match in channel.Matches)
                            {
                                if (match.Clan.Id == player.ClanId)
                                    this.Field0.Add(match);
                            }
                        }
                    }
                }
                this.Client.SendPacket(new PROTOCOL_CS_CLAN_MATCH_RESULT_LIST_ACK(player.ClanId == 0 ? 91 : 0, this.Field0));
            }
            catch (Exception ex)
            {
