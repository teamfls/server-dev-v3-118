    ...
    public class PROTOCOL_CS_REPLACE_INTRO_REQ : GameClientPacket
    {
        ...
        public override void Run()
        {
            try
            {
                ...
                if (clan.Id > 0 && clan.Info != this.Field0 && (clan.OwnerId == this.Client.PlayerId || player.ClanAccess >= 1 && player.ClanAccess <= 2))
                {
                    if (!ComDiv.UpdateDB("system_clan", "info", (object)this.Field0, "id", (object)clan.Id))
                        this.Field1 = EventErrorEnum.CLAN_REPLACE_INTRO_ERROR;
                    else
                        clan.Info = this.Field0;
                }
                else
                    this.Field1 = EventErrorEnum.CLAN_FAILED_CHANGE_OPTION;
                this.Client.SendPacket(new PROTOCOL_CS_REPLACE_INTRO_ACK(this.Field1));
            }
            catch (Exception ex)
            {
