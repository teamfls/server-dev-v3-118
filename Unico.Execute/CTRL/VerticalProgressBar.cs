...
namespace Executable.CTRL
{
	...
	public sealed class VerticalProgressBar : System.Windows.Forms.UserControl
	{
		...
		private void drawBorder(Graphics dc)
		{
			...
			{
				Color darkColor = ControlPaint.Dark(BackColor);
				Color brightColor = ControlPaint.Dark(BackColor);
				Pen p = new Pen(darkColor, 1);
				dc.DrawLine(p, Width, 0, 0, 0);
				dc.DrawLine(p, 0, 0, 0, Height);
				p = new Pen(brightColor, 1);
				dc.DrawLine(p, 0, Height, Width, Height);
				dc.DrawLine(p, Width, Height, Width, 0);
			}
		}
		...
		private void drawBar(Graphics dc)
		{
			...
			height = (m_Value - m_Minimum) * Height / (m_Maximum - m_Minimum); // the bar height
